...
skale node init --github-token $TOKEN --git-branch $BRANCH --docker-username $DOCKER_USERNAME --docker-password $DOCKER_PASSWORD --db-password $DB_PASSWORD --disk-mountpoint $DISK_MOUNTPOINT --install-deps
